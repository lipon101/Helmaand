...
8. **Auth Bypass SQLi** -- The staff login builds `SELECT * FROM auth_user WHERE username='{username}' AND password='{password}' AND is_staff=1`. The WAF blocks SQLi in URLs and POST bodies, but the login form has JavaScript that automatically encodes your input into a **cookie** before submit (WAF-invisible). Just type the SQLi directly in the username field:
   ```
   Username: admin' OR '1'='1
   Password: anything
   ```
   Click "Staff Login" — the JS handles the WAF bypass transparently. The flag appears in a flash message.
