...
def lab_index(request):
    ...
    challenges = [
        ...
        },
        {
            ...
            'category': 'XSS',
            'endpoint': '/newsletter/',
            'description': 'The newsletter signup reflects the ?name= GET parameter into an HTML attribute without escaping quotes. Break out of the attribute and inject an event handler.',
            'hint': 'The name is placed inside <code>&lt;input value="NAME"&gt;</code>. Try <code>?name=" onmouseover="alert(document.cookie)</code> to escape the attribute and fire an event handler.',
            'objective': 'Escape the HTML attribute context and execute JavaScript via an event handler.',
            'flag_hint': 'A flag cookie is set on the newsletter page. Trigger your injected event handler to read it.',
        },
        {
            'id': 'xss_self',
            'name': 'Self-XSS',
            ...
            'description': 'The profile page renders the logged-in user\'s username directly into the page without escaping, and sets a flag cookie. A user can set a payload as their own username to execute JS on their own profile page.',
            'hint': 'The username is rendered with |safe in the profile page. Register or change to a username containing <code>&lt;img src=x onerror="alert(document.cookie)"&gt;</code>, then visit your profile.',
            'objective': 'Execute JavaScript on your own profile page by injecting a payload into your username.',
            'flag_hint': 'A flag cookie is set on the profile page. Your injected payload can read it via document.cookie.',
        },
        # ──────────────────── SQLi (5) ────────────────────
        {
            'id': 'sqli_union',
        ...
        },
        {
            ...
            'difficulty': 'Intermediate',
            'category': 'CSRF',
            'endpoint': '/accounts/quick-login/',
            'description': 'The quick-login endpoint is @csrf_exempt with no CSRF token. An attacker can forge a cross-site login to log the victim into the attacker\'s own account.',
            'hint': 'Build a page with an auto-submitting form that POSTs the attacker\'s credentials to /accounts/quick-login/. When the victim visits, they are logged in as the attacker.',
            'objective': 'Log a victim into the attacker\'s account via a forged cross-site login request.',
            'flag_hint': 'After the forged login succeeds, the redirect page shows the flag in a flash message.',
        },
        {
            'id': 'csrf_logout',
            'name': 'Logout CSRF',
            'difficulty': 'Easy',
            'category': 'CSRF',
            'endpoint': '/accounts/force-logout/',
            'description': 'The force-logout endpoint logs out the current user via a plain GET request with no CSRF protection. A victim can be forcibly logged out by visiting a crafted link or image tag.',
            'hint': 'Embed <code>&lt;img src="https://helmaand.local/accounts/force-logout/"&gt;</code> on any page. When a logged-in victim visits, they are silently logged out.',
            'objective': 'Forcefully log out a victim user via a cross-site GET request.',
            'flag_hint': 'After the forced logout, the login page shows the flag in a flash message.',
        },
        {
            'id': 'csrf_password',
