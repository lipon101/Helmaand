...
def lab_index(request):
    ...
    challenges = [
        ...
        # ──────────────────── CSRF (3) ────────────────────
        {
            ...
            'description': 'The email-change endpoint disables CSRF protection via @csrf_exempt. Normal form submissions (with CSRF token) work but don\'t reveal the flag. You must craft a cross-site POST without a CSRF token to get the flag.',
            'hint': 'The form has a CSRF token for legitimate use, but the view is @csrf_exempt — it accepts POSTs without a token. Build an external HTML page with an auto-submitting form that does NOT include a CSRF token. Submit to /accounts/change-email/ with email=attacker@evil.com.',
            'objective': 'Change a victim user\'s email via a forged cross-site POST without a CSRF token.',
            'flag_hint': 'The flag appears only when the POST is received without a CSRF token (simulating cross-origin).',
        },
        {
            'id': 'csrf_get',
            'name': 'CSRF via GET',
            ...
            'description': 'The reset-preferences endpoint performs a state-changing operation (clearing shipping info) via a plain GET request. The flag only appears when the request comes without a Referer header — simulating cross-origin access.',
            'hint': 'Embed <code>&lt;img src="https://helmaand.local/accounts/reset-preferences/"&gt;</code> on an external page, or type the URL directly in your browser (no Referer). The state change happens silently, and the flag appears on the redirect.',
            'objective': 'Reset a victim user\'s shipping profile data via a cross-site GET request.',
            'flag_hint': 'The flag only appears when the request has no Referer header (external page or direct URL).',
        },
        {
            'id': 'csrf_password',
            ...
            'description': 'The password-change endpoint is @csrf_exempt — it accepts POSTs without a CSRF token. You must forge a cross-site request without a token to change the victim\'s password and get the flag.',
